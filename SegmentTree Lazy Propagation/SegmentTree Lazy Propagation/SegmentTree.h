...
private:
	...
	void UpdateTree(unsigned Node, unsigned Left, unsigned Right, unsigned start, unsigned end, T val);
...
public:
	...
	*/
	...
	void UpdateRange(unsigned start, unsigned end, T value);
	/*
		Description: wrapper for UpdateTree function. Sets the elements belonging to
					 [start, end] to the given value
					 
		throws: exception if start or end is invalid, or if start > end
